  ...
  return (
    <>
      ...
      <NavLink to={`/product/${product_id}`}>
        <div className="relative flex w-full md:w-full flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md group">
          <div className={`"relative mx-3 mt-3 flex h-60 overflow-hidden rounded-xl justify-center bg-cover bg-center backdrop-blur-xl `} style={{ backgroundImage: product_feature_img ? `url(${product_feature_img})` : 'none', backgroundSize: 'cover', backgroundPosition: 'center' }}>
            {/* <img className="object-cover transition-all ease-linear duration-300 group-hover:scale-110" src={product_feature_img} alt={title} loading='lazy' minHeight={120} minWidth={200} /> */}
          ...
          <div className="mt-4 px-5 pb-5">
              <h2 className="title text-xl tracking-tight text-slate-900">{title}</h2>
